...
resource "aws_s3_bucket_policy" "rules" {
  bucket = aws_s3_bucket.rules.id
    ...
    Statement = [
      {
        ...
        Principal = "*"
        Action    = "s3:GetObject"
        Resource  = "${aws_s3_bucket.rules.arn}/templates/*"
      }
    ]
  })
...
# Upload CloudFormation template for customer onboarding
resource "aws_s3_object" "customer_onboarding_template" {
  bucket       = aws_s3_bucket.rules.id
  key          = "templates/customer-onboarding-role.yaml"
  source       = "${path.module}/customer-onboarding-role.yaml"
  etag         = filemd5("${path.module}/customer-onboarding-role.yaml")
  content_type = "text/yaml"

  tags = merge(local.common_tags, {
    Name = "Customer Onboarding CloudFormation Template"
  })
}
